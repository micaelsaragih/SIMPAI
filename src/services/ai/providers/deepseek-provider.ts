...
if (typeof window !== "undefined") {
  throw new Error("DeepSeekAIProvider can only be instantiated on the server side.")
}

export class DeepSeekAIProvider implements IAIProvider {
  name = "DeepSeek"

  // Model strategy: Primary -> Fallback (DeepSeek only exposes deepseek-chat primarily)
  private models = [
    "deepseek-chat"
  ]
  ...
  async analyzeArticle(request: AnalysisRequest): Promise<AnalysisResponse> {
    ...
    for (const model of this.models) {
      try {
        console.log(`[DeepSeek] Attempting academic review using model: ${model}`)
        ...
        console.log(`[DeepSeek] Successfully received response from ${model} in ${duration}ms`)
        ...
        console.warn(`[DeepSeek Fallback] Model ${model} failed. Trying next model. Error:`, error)
        lastError = error instanceof Error ? error : new Error(String(error))
      }
    }

    throw new Error(`DeepSeek analysis failed for all models in the chain. Last error: ${lastError?.message}`)
  }
  ...
  private async fetchWithRetryAndTimeout(
    ...
    while (attempts < maxRetries) {
      ...
      try {
        ...
        if (!content) {
          throw new Error("Empty completion content received from DeepSeek API.")
        }

        // Logging tokens usage
        if (data.usage) {
          console.log(`[DeepSeek Usage Logs] Provider: DeepSeek | Model: ${model} | Prompt Tokens: ${data.usage.prompt_tokens} | Completion Tokens: ${data.usage.completion_tokens} | Total Tokens: ${data.usage.total_tokens}`)
        }
        ...
        console.warn(`[DeepSeek Retry Log] Attempt ${attempts}/${maxRetries} failed for model ${model}. Error: ${err.message}`)
    ...
    throw new Error("Max retries reached on DeepSeek connection.")
  }
  ...
  private parseAndValidateResponse(text: string): AIAnalysisResult {
    try {
      ...
      console.error("[DeepSeek Schema Validation Error] Failed to parse or validate JSON response:", err)
