...
if (typeof window !== "undefined") {
  throw new Error("GeminiAIProvider can only be instantiated on the server side.")
}
...
export class GeminiAIProvider implements IAIProvider {
  ...
  async analyzeArticle(request: AnalysisRequest): Promise<AnalysisResponse> {
    ...
    for (const model of this.models) {
      try {
        console.log(`[Gemini] Attempting academic review using model: ${model}`)
        ...
        console.log(`[Gemini] Successfully received response from ${model} in ${duration}ms`)
        ...
        console.warn(`[Gemini Fallback] Model ${model} failed. Trying next model. Error:`, error)
        lastError = error instanceof Error ? error : new Error(String(error))
      }
    }

    throw new Error(`Gemini analysis failed for all models in the chain. Last error: ${lastError?.message}`)
  }
  ...
  private async fetchWithRetryAndTimeout(
    ...
    while (attempts < maxRetries) {
      ...
      try {
        ...
        if (!content) {
          throw new Error("Empty completion content received from Gemini API.")
        }

        // Logging tokens usage
        if (data.usage) {
          console.log(`[Gemini Usage Logs] Provider: Gemini | Model: ${model} | Prompt Tokens: ${data.usage.prompt_tokens} | Completion Tokens: ${data.usage.completion_tokens} | Total Tokens: ${data.usage.total_tokens}`)
        }
        ...
        console.warn(`[Gemini Retry Log] Attempt ${attempts}/${maxRetries} failed for model ${model}. Error: ${err.message}`)
    ...
    throw new Error("Max retries reached on Gemini connection.")
  }
  ...
  private parseAndValidateResponse(text: string): AIAnalysisResult {
    try {
      ...
      console.error("[Gemini Schema Validation Error] Failed to parse or validate JSON response:", err)
